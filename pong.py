...
def game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate, score_to_win, display,df):
    score = [0, 0]
    #these are lists with data about where the pad is where my pad is, where ball is and what is the current score
    comp_pad=[]
    my_pad = []
    ball_ = []
    score_ = []
    ...
    while max(score) < score_to_win:
        ...
        comp_pad.append(paddles[0].frect.pos)
        my_pad.append(paddles[1].frect.pos)

        ball_.append(ball.frect.pos)
        score_.append(old_score)
    ...
    #print(my_pad)
    df["my pad"] = my_pad
    df["comp pad"] = comp_pad
    df["ball"] = ball_
    df["score"] = score_
    #print(type(df["my pad"][0]))
    font = pygame.font.Font(None, 64)
    last_line = pd.DataFrame([[paddles[1].frect.pos,paddles[0].frect.pos,ball.frect.pos,score]],columns=["my pad","comp pad", "ball","score"],index=[0])
...
def init_game():
    ...
    df = game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate, score_to_win, 1,df)
    print(df)
